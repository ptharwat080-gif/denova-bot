...
/**
 * Tries to pull structured booking details (name, phone, service, date, time) out of
 * a conversation transcript. Returns null if any of the 5 fields isn't clearly present yet
 * (still mid-conversation) - the caller should fall back to just logging raw notes in that case.
 *
 * @param {string} conversationText - the recent conversation, one line per turn
 * @returns {Promise<{name:string, phone:string, service:string, date:string, time:string}|null>}
 */
