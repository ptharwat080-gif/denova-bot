...
const { SYSTEM_PROMPT, NO_REPLY_SENTINEL } = require("./knowledge");
 ...
 * @returns {Promise<string|null>} the AI-generated reply text, or null if the message was
 *   completely unrelated to the clinic (see NO_REPLY_SENTINEL in knowledge.js) - callers must
 *   treat null as "send nothing back to the customer".
 */
async function getAiReply(userMessage, history = [], lang) {
  ...
  const reply = textBlock ? textBlock.text.trim() : "معلش، ممكن تعيد سؤالك بطريقة تانية؟";
  return reply === NO_REPLY_SENTINEL ? null : reply;
}
...
Important for the "service" field: if the customer mentioned MORE THAN ONE thing they're interested in (for example a root canal AND a teeth whitening session), include ALL of them in the "service" field, separated by " / " - do not drop any of them and do not collapse them into just one.
...
/**
 * Advisor mode: used only for the staff advisor WhatsApp number in server.js. Instead of
 * replying to a customer directly, the clinic staff sends in a real question they got from a
 * customer (on a call, in person, or any channel not wired to the bot), and this returns a
 * ready-to-copy suggested reply written in the exact same voice/persona as the customer-facing
 * bot (SYSTEM_PROMPT), so staff never have to guess how "the bot" would have answered it.
 *
 * @param {string} staffMessage - the question/situation the staff member typed in
 * @returns {Promise<string|null>} the suggested reply text, or null if nothing usable came back
 */
async function getAdvisorSuggestion(staffMessage) {
  if (!API_KEY) {
    throw new Error("ANTHROPIC_API_KEY is not set");
  }
...
ملحوظة مهمة: انت دلوقتي مش بترد على عميل مباشرة - موظف/ة العيادة بيبعتلك سؤال وصله من عميل حقيقي (تليفون، واتساب تاني، شخصيًا، أو أي قناة تانية) وعايز اقتراح رد جاهز يقدر ينسخه ويبعته للعميل زي ما هو. اتبع بالظبط نفس قواعد اللغة والأسلوب ومهارة الإقناع الاحترافي فوق في صياغة الرد المقترح. رد بنص الرسالة المقترحة بس - من غير أي مقدمة زي "اقتراحي هو" أو "ممكن ترد بـ" أو أي تعليق إضافي حواليها.`;
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 700,
      system: systemPrompt,
      messages: [{ role: "user", content: staffMessage }],
    }),
  });
...
module.exports = { getAiReply, extractBookingDetails, getAdvisorSuggestion };
