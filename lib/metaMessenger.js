...
/**
 * Best-effort fetch of the sender's real display name via Meta's Graph API. The messaging
 * webhook payload itself NEVER includes a name - only an opaque PSID (Messenger) or IGSID
 * (Instagram) - unlike WhatsApp, which sends the contact's profile name directly in every
 * webhook. Without this extra call, every Messenger/Instagram sheet row stays nameless unless
 * the customer happens to type their name in the chat itself.
 *
 * Never throws: returns "" on any failure, including the common case where the app hasn't
 * cleared Meta's App Review yet (see README) - some permission/verification states make this
 * endpoint return an error or an empty profile even though messaging itself still works. A
 * failed name lookup should never block logging the lead.
 *
 * @param {"messenger"|"instagram"} platform
 * @param {string} senderId - PSID (Messenger) or IGSID (Instagram)
 * @returns {Promise<string>} the customer's display name, or "" if unavailable
 */
async function getMetaUserName(platform, senderId) {
  try {
    if (platform === "instagram") {
      if (!INSTAGRAM_ACCESS_TOKEN) return "";
      const url = `https://graph.instagram.com/${IG_GRAPH_VERSION}/${senderId}?fields=name,username&access_token=${INSTAGRAM_ACCESS_TOKEN}`;
      const res = await fetch(url);
      if (!res.ok) return "";
    ...
    const url = `https://graph.facebook.com/${GRAPH_VERSION}/${senderId}?fields=first_name,last_name&access_token=${PAGE_ACCESS_TOKEN}`;
    const res = await fetch(url);
    if (!res.ok) return "";
    const data = await res.json();
    return [data.first_name, data.last_name].filter(Boolean).join(" ");
  } catch (err) {
    console.error(`getMetaUserName(${platform}) failed:`, err.message);
    return "";
  }
}
...
module.exports = { sendMetaText, parseMetaWebhookEvents, sendCommentReply, sendPrivateReply, getMetaUserName };
