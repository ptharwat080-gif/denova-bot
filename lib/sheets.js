...
// رقم مختلف اتقال أثناء الحجز (لو فيه) | مواضيع تانية اتكلم فيها العميل | نص المحادثة كاملة
...
function buildRowValues(lead, dateStr) {
  return [
    ...
    lead.notes || "",
    lead.altPhone || "", // L: a different phone number the customer gave while booking, if it doesn't match column C
    lead.otherTopics || "", // M: other subjects/questions the customer raised in the chat besides the booking itself
    lead.transcript || "", // N: the full conversation so far, so the clinic can review any chat directly from the sheet
  ];
}

async function writeRow(rowNumber, values, accessToken) {
  const writeRange = encodeURIComponent(`'${SHEET_TAB}'!A${rowNumber}:N${rowNumber}`);
...
/**
 * Reads every existing lead row back out of the sheet (columns A:N, from FIRST_DATA_ROW to the
 * last filled row), so the server can rebuild its in-memory conversation state after a restart
 * or redeploy - which otherwise wipes the WhatsApp dashboard's message history, since that state
 * normally only lives in RAM. The sheet is the one thing that's already durable and free, so we
 * reuse it as the persistence layer instead of adding a paid database/disk.
 *
 * @returns {Promise<Array<{row:number, name:string, phone:string, source:string,
 *   packageInterest:string, status:string, appointmentDate:string, appointmentTime:string,
 *   notes:string, altPhone:string, otherTopics:string, transcript:string}>>}
 */
async function loadAllLeadRows() {
  if (!SHEET_ID) return [];

  const accessToken = await getAccessToken();
  const range = encodeURIComponent(`'${SHEET_TAB}'!A${FIRST_DATA_ROW}:N`);
  ...
  return rows.map((cols, i) => ({
    ...
    phone: cols[2] || "",
    source: cols[3] || "",
    packageInterest: cols[4] || "",
    status: cols[5] || "",
    appointmentDate: cols[6] || "",
    appointmentTime: cols[7] || "",
    notes: cols[10] || "",
    altPhone: cols[11] || "",
    otherTopics: cols[12] || "",
    transcript: cols[13] || "",
...
module.exports = { appendLead, updateLeadRow, loadAllLeadRows };
